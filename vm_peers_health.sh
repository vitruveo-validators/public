...
WEBHOOK_URL="https://discord.com/api/webhooks/1288522343290900500/8PilE2d1jCKdgP74mUTGo3W8qIwU3OjI6ILaDwQcPuIQO725L6ug_v-5btAdlhZyrE8B"
USER_ID=""

# CPU Health
CPU_USAGE=$(top -bn1 | grep "Cpu(s)" | sed "s/.*, *\([0-9.]*\)%* id.*/\1/" | awk '{print 100 - $1}')
PERCENTAGE=$(printf "%.2f" "$CPU_USAGE")
...
if [ "$USAGE_PERCENT" -gt 90 ]; then
    send_discord_message "<@$USER_ID> Critical❗Node disk at ${USAGE_PERCENT}% - **Action immediately**❗\nTotal Size:${TOTAL_SIZE}B\nUsed: ${USED_SIZE}\n**Available: ${AVAIL_SIZE}**"
...
if [ "$PEER_COUNT" -eq 0 ]; then
    send_alert "<@${USER_ID}> ❗ Critical: NO peers connected to your Validator!"
